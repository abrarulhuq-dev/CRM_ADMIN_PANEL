...
const StaffForm = () => {
    ...
    const { backendurl, manager, token } = useAppcontext()
    ...
    const onsubmithandler = async (e) => {
        ...
        try {
            ...
            formdata.append('manager', selected)
            ...
            const { data } = await axios.post(backendurl + 'api/staff/', formdata, { headers: { Authorization: `Bearer ${token}` } })
    ...
    return (
        <form className=' w-1/2 mx-auto mt-10 bg-white p-8 rounded-xl' onSubmit={onsubmithandler}>
            ...
            <div className='md:grid md:grid-cols-2 gap-5'>
                ...
                <div className='mb-2'>
                    ...
                    <div className="flex flex-col text-sm relative ">
                        <button type="button" onClick={() => setIsOpen(!isOpen)} className="w-full text-left px-4 pr-2 py-4  rounded  text-gray-800 bg-gray-200">
                            <span className='text-gray-700'>{manager.find(d => d.id === selected)?.name || "Select"}</span>
                        ...
                        {isOpen && (
                            <ul className= "w-full h-50 absolute top-12 bg-white border border-gray-300 rounded shadow-md mt-1 py-2 overflow-y-scroll no-scollbar">
                                {manager.map((manager) => (
                                    <li key={manager.id} className="px-4 py-2 hover:bg-indigo-500 hover:text-white cursor-pointer" onClick={() => handleSelect(manager.id)} >
                                        {manager.name}
                ...
                <div className='mb-4'>
                    <p className='text-gray-500 font-medium mb-2'>Skill</p>
                    <input className='w-full px-4 py-4 bg-gray-200 rounded outline-none text-gray-700' type='text' placeholder='Skill' onChange={(e) => setSkill(e.target.value)} />
                </div>
            </div>
            <div className=' flex flex-wrap-reverse gap-4 justify-end'>
                <button className={`bg-gray-200 text-black px-6 py-2 rounded mt-4 ml-4 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`} onClick={() => navigate('/staffs')}>Cancel</button>
